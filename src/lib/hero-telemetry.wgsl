...
fn computerNode(p: vec2f) -> f32 {
  let q = rotate(p, -0.14);
  let bezel = sdRoundedBox(q, vec2f(0.18, 0.12), 0.025);
  let screen = sdRoundedBox(q, vec2f(0.155, 0.092), 0.014);
  var mark = opSmoothSub(bezel, screen, 0.008);
  let stand = sdRoundedBox(q - vec2f(0.0, 0.15), vec2f(0.05, 0.025), 0.006);
  let base = sdRoundedBox(q - vec2f(0.0, 0.19), vec2f(0.11, 0.012), 0.006);
...
fn fs_main(@location(0) uv: vec2f) -> @location(0) vec4f {
  ...
  let fortinetRed = vec3f(0.854902, 0.160784, 0.109804);
  let paper = vec3f(1.0, 0.980392, 0.945098);
  let leftClear = smoothstep(0.34, 0.62, uv.x);
  var a = fill * 0.08 + line * 0.28 + glow * 0.08 + traces * 0.32 + spark * 0.06;
  a *= 0.55 * leftClear;
  a = clamp(a, 0.0, 0.34);
  let col = mix(fortinetRed, paper, spark * 0.45 + line * 0.05);
